...
let rec add_signature env root rel_opt signat =
  ...
  let f env item =
    match item with
      Types.Sig_value {Types.val_id={Types.id_name=ident}} -> { env with env_values = (rel_name ident, qualify ident) :: env.env_values }
    | Types.Sig_type {Types.tcs_id={Types.id_name=ident}} -> { env with env_types = (rel_name ident, qualify ident) :: env.env_types }
