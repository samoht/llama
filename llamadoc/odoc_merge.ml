...
let rec merge_module_types merge_options mli ml =
  ...
  List.iter
    (fun ex ->
      try
        let _ = List.find
            ...
            (List.rev (Odoc_module.module_type_elements true ml))
        in
        ()
      with
        Not_found ->
    ...
    (Odoc_module.module_type_exceptions true mli);
  (* merge types *)
  List.iter
    (fun ty ->
      try
    ...
    (Odoc_module.module_type_types true mli);
  ...
  List.iter
    ...
    (Odoc_module.module_type_values true mli)
...
and merge_modules merge_options mli ml =
  ...
  List.iter
    (fun ex ->
      try
        let _ = List.find
            ...
            (* we look for the last exception with this name defined in the implementation *)
            (List.rev (Odoc_module.module_elements true ml))
        in
        ()
      with
        Not_found ->
    ...
    (Odoc_module.module_exceptions true mli);
  (* merge types *)
  List.iter
    (fun ty ->
      try
    ...
    (Odoc_module.module_types true mli);
  (* A VOIR : merge included modules ? *)
  ...
  List.iter
    (fun v ->
      try
        let _ = List.find
            ...
            (* we look for the last value with this name defined in the implementation *)
            (List.rev (Odoc_module.module_values true ml))
        in
        ()
      with
        Not_found ->
    ...
    (Odoc_module.module_values true mli);
