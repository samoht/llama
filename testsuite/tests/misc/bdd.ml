...
let htab       = ref(Array.create (!sz_1+1) ([] : bdd list))
let n_items    = ref 0
...
let main () =
  let n =
    if Array.length Sys.argv >= 2 then int_of_string Sys.argv.(1) else (* 22 *) 11 in
  let ntests =
    if Array.length Sys.argv >= 3 then int_of_string Sys.argv.(2) else (* 100 *) 50 in
  let bdd = hwb n in
