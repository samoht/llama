...
let filter_matrix matcher pss =
  ...
  let rec filter_rec = function
        ...
        | _ ->
            begin
              ...
              | OrPat   ->
                ...
                | Pat_or (p1,p2,_) -> filter_rec [(p1::ps) ;(p2::ps)]@rem
                | _ -> assert false
            end
        end
    | [] -> []
    | _ ->
        pretty_matrix pss ;
        Fatal.error "Matching.filter_matrix" in
  filter_rec pss
...
let ctx_matcher p =
  ...
  match p.pat_desc with
  | Pat_construct (cstr,omegas) ->
      (fun q rem -> match q.pat_desc with
      | Pat_construct (cstr',args) when cstr==cstr' ->
          p,args @ rem
      | Pat_any -> p,omegas @ rem
      | _ -> raise NoMatch)
  | Pat_constant cst ->
      (fun q rem -> match q.pat_desc with
      | Pat_constant cst' when cst=cst' ->
          p,rem
      | Pat_any -> p,rem
      | _ -> raise NoMatch)
  | Pat_variant (lab,Some omega,_) ->
      (fun q rem -> match q.pat_desc with
      | Pat_variant (lab',Some arg,_) when lab=lab' ->
          p,arg::rem
      | Pat_any -> p,omega::rem
      | _ -> raise NoMatch)
  | Pat_variant (lab,None,_) ->
      (fun q rem -> match q.pat_desc with
      | Pat_variant (lab',None,_) when lab=lab' ->
          p,rem
      | Pat_any -> p,rem
      | _ -> raise NoMatch)
  | Pat_array omegas ->
      ...
      (fun q rem -> match q.pat_desc with
      | Pat_array args when List.length args=len ->
          p,args @ rem
      | Pat_any -> p, omegas @ rem
      | _ -> raise NoMatch)
  | Pat_tuple omegas ->
      (fun q rem -> match q.pat_desc with
      | Pat_tuple args -> p,args @ rem
      | _          -> p, omegas @ rem)
  | Pat_record l -> (* Records are normalized *)
      (fun q rem -> match q.pat_desc with
      | Pat_record l' ->
  ...
  | Pat_lazy omega ->
      (fun q rem -> match q.pat_desc with
      | Pat_lazy arg -> p, (arg::rem)
      | _          -> p, (omega::rem))
 | _ -> Fatal.error "Matching.ctx_matcher"
...
let filter_ctx q ctx =
  ...
  let rec filter_rec = function
    ...
    | [] -> []
    | _ ->  Fatal.error "Matching.filter_ctx" in
...
let ctx_lub p ctx =
  List.fold_right
    (fun {left=left ; right=right} r ->
      ...
      | _ -> Fatal.error "Matching.ctx_lub")
    ctx []
...
let simplify_or p =
  let rec simpl_rec p = match p with
    | {pat_desc = Pat_any|Pat_var _} -> raise (Var p)
    | {pat_desc = Pat_alias (q,id)} ->
        begin try
          {p with pat_desc = Pat_alias (simpl_rec q,id)}
        with
        | Var q -> raise (Var {p with pat_desc = Pat_alias (q,id)})
        end
    | {pat_desc = Pat_or (p1,p2,o)} ->
        let q1 = simpl_rec p1 in
        begin try
          let q2 = simpl_rec p2 in
          {p with pat_desc = Pat_or (q1, q2, o)}
        with
        | Var q2 -> raise (Var {p with pat_desc = Pat_or (q1, q2, o)})
        end
    | {pat_desc = Pat_record lbls} ->
        let all_lbls = all_record_args lbls in
        {p with pat_desc=Pat_record all_lbls}
    | _ -> p in
  try
    simpl_rec p
  with
...
| (arg,_)::_ ->
    let rec simplify = function
      ...
      | ((pat :: patl, action) as cl) :: rem ->
          ...
          | Pat_record lbls ->
              let all_lbls = all_record_args lbls in
              let full_pat = {pat with pat_desc=Pat_record all_lbls} in
...
| ({pat_desc=Pat_any} :: _, _) :: rem -> what_is_cases rem
| (({pat_desc=(Pat_var _|Pat_or (_,_,_)|Pat_alias (_,_))}::_),_)::_
...
| Pat_record lpats ->
    List.fold_left
      ...
      r lpats
| Pat_construct (_,pats) ->
    List.fold_left extract_vars r pats
| Pat_array pats ->
    List.fold_left extract_vars r pats
| Pat_variant (_,Some p, _) -> extract_vars r p
| Pat_lazy p -> extract_vars r p
| Pat_or (p,_,_) -> extract_vars r p
| Pat_constant _|Pat_any|Pat_variant (_,None,_) -> r
...
let group_constant = function
  | {pat_desc= Pat_constant _} -> true
  | _                           -> false

and group_constructor = function
  | {pat_desc = Pat_construct (_, _)} -> true
  | _ -> false

and group_variant = function
  | {pat_desc = Pat_variant (_, _, _)} -> true
  | _ -> false

and group_var = function
  | {pat_desc=Pat_any} -> true
  | _ -> false

and group_tuple = function
  | {pat_desc = (Pat_tuple _|Pat_any)} -> true
  | _ -> false

and group_record = function
  | {pat_desc = (Pat_record _|Pat_any)} -> true
  | _ -> false

and group_array = function
  | {pat_desc=Pat_array _} -> true
  | _ -> false

and group_lazy = function
  | {pat_desc = Pat_lazy _} -> true
  | _ -> false

let get_group p = match p.pat_desc with
| Pat_any -> group_var
| Pat_constant _ -> group_constant
| Pat_construct (_, _) -> group_constructor
| Pat_tuple _ -> group_tuple
| Pat_record _ -> group_record
| Pat_array _ -> group_array
| Pat_variant (_,_,_) -> group_variant
| Pat_lazy _ -> group_lazy
|  _ -> Fatal.error "Matching.get_group"
...
and split_constr cls args def k =
  ...
  | Pat_any -> precompile_var args cls def k
  | _ ->
...
| _  ->
    let rec do_cases = function
      | ({pat_desc=Pat_or _} as orp::patl, action)::rem ->
...
| Pat_constant c1 when c1=cst -> rem
| Pat_any                     -> rem
| _ -> raise NoMatch

let get_key_constant caller = function
  | {pat_desc= Pat_constant cst} -> cst
  | p ->
...
let make_constant_matching p def ctx = function
    [] -> Fatal.error "Matching.make_constant_matching"
  | (_ :: argl) ->
      let def =
        make_default
...
let get_key_constr = function
  | {pat_desc=Pat_construct (cstr,_)} -> cstr
  | _ -> assert false

let get_args_constr p rem = match p with
| {pat_desc=Pat_construct (_,args)} -> args @ rem
| _ -> assert false

let pat_as_constr = function
  | {pat_desc=Pat_construct (cstr,_)} -> cstr
  | _ -> Fatal.error "Matching.pat_as_constr"
...
| 0 ->
    ...
    | Pat_construct (cstr1, []) when cstr == cstr1 ->
        rem
    | Pat_any -> rem
    | _ -> raise NoMatch in
    matcher_rec
...
pat_desc = Pat_or (a1, a2, None)}::
            rem
        | _, _ -> assert false
        end
    | Pat_construct (cstr1, [arg]) when cstr == cstr1 ->
        arg::rem
    | Pat_any -> omega::rem
    | _ -> raise NoMatch in
    matcher_rec
| _ ->
    fun q rem -> match q.pat_desc with
    | Pat_or (_,_,_) -> raise OrPat
    | Pat_construct (cstr1, args)
        when cstr == cstr1 -> args @ rem
    | Pat_any -> Parmatch.omegas (cs_arity cstr) @ rem
    | _        -> raise NoMatch

let make_constr_matching p def ctx = function
    [] -> Fatal.error "Matching.make_constr_matching"
  | ((arg, mut) :: argl) ->
...
| Pat_variant (lab1,_,_) when lab1=lab -> rem
| Pat_any -> rem
| _   -> raise NoMatch


let make_variant_matching_constant p lab def ctx = function
    [] -> Fatal.error "Matching.make_variant_matching_constant"
  | ((arg, mut) :: argl) ->
...
| Pat_or (_,_,_) -> raise OrPat
| Pat_variant (lab1,Some arg,_) when lab1=lab -> arg::rem
| Pat_any -> omega::rem
| _   -> raise NoMatch


let make_variant_matching_nonconst p lab def ctx = function
    [] -> Fatal.error "Matching.make_variant_matching_nonconst"
  | ((arg, mut) :: argl) ->
...
(*
match p.pat_desc with
| Pat_variant(lab, Some _ , _) ->  Tag_block (Typeutil.hash_variant lab)
| Pat_variant(lab, None , _) -> Tag_constant (Typeutil.hash_variant lab)
|  _ -> assert false
*)
...
(*
  ...
  let rec divide = function
      ({pat_desc = Pat_variant(lab, pato, _)} as p:: patl, action) :: rem ->
...
let make_var_matching def = function
  | [] ->  Fatal.error "Matching.make_var_matching"
  | _::argl ->
      {cases=[] ;
        args = argl ;
...
| {pat_desc = Pat_any} -> omega :: rem
| {pat_desc = Pat_lazy arg} -> arg :: rem
| _ ->  assert false

let matcher_lazy p rem = match p.pat_desc with
| Pat_or (_,_,_)     -> raise OrPat
| Pat_var _          -> get_arg_lazy omega rem
...
| {pat_desc = Pat_any} -> omegas arity @ rem
| {pat_desc = Pat_tuple args} ->
    args @ rem
| _ ->  assert false

let matcher_tuple arity p rem = match p.pat_desc with
| Pat_or (_,_,_)     -> raise OrPat
| Pat_var _          -> get_args_tuple arity omega rem
| _                   ->  get_args_tuple arity p rem

let make_tuple_matching arity def = function
    [] -> Fatal.error "Matching.make_tuple_matching"
  | (arg, mut) :: argl ->
      let rec make_args pos =
        if pos >= arity
        then argl
...
let get_key_array = function
  | {pat_desc=Pat_array patl} -> List.length patl
  | _ -> assert false

let get_args_array p rem = match p with
| {pat_desc=Pat_array patl} -> patl@rem
| _ -> assert false

let matcher_array len p rem = match p.pat_desc with
| Pat_or (_,_,_) -> raise OrPat
| Pat_array args when List.length args=len -> args @ rem
| Pat_any -> Parmatch.omegas len @ rem
| _ -> raise NoMatch

let make_array_matching kind p def ctx = function
  | [] -> Fatal.error "Matching.make_array_matching"
  | ((arg, mut) :: argl) ->
...
let rec do_tests_nofail tst arg = function
  | [] -> Fatal.error "Matching.do_tests_nofail"
  | [_,act] -> act
  | (c,act)::rem ->
      Lifthenelse
        (Lprim (tst, [arg ; Lconst (Const_base c)]),
...
let rec list_as_pat = function
  | [] -> Fatal.error "Matching.list_as_pat"
  | [pat] -> pat
  | pat::rem ->
      {pat with pat_desc = Pat_or (pat,list_as_pat rem,None)}
...
let combine_constructor modenv arg ex_pat cstr partial ctx def
        ...
        (fun (ex, act) rem ->
          ...
          | Tag_exception _ ->
              Lifthenelse(Lprim(Pintcomp Ceq,
                                [Lprim(Pfield 0, [arg]); Makelambda.transl_exception modenv ex]),
                          act, rem)
          | _ -> assert false)
        tests default in
...
| _        -> Fatal.error "Matching.comp_exit"
...
let rec compile_match modenv repr partial ctx m = match m with
| { cases = [] } -> comp_exit ctx m
| { cases = ([], action) :: rem } ->
    if is_guarded action then begin
      let (lambda, total) =
        compile_match modenv None partial ctx { m with cases = rem } in
...
| { args = (arg, str)::argl } ->
    ...
    let (lam, total) =
      comp_match_handlers
        (do_compile_matching modenv repr) partial ctx newarg first_match rem in
...
and do_compile_matching_pr repr partial ctx arg x =
  ...
  r
*)
and do_compile_matching modenv repr partial ctx arg pmh = match pmh with
| Pm pm ->
  let pat = what_is_cases pm.cases in
  begin match pat.pat_desc with
  | Pat_any ->
      compile_no_test modenv
  ...
  | Pat_record ((lbl,_)::_) ->
      compile_no_test modenv
        (divide_record (Pmc_pattern.calc_lbl_all lbl) (normalize_pat pat))
        ctx_combine repr partial ctx pm
  | Pat_constant cst ->
      compile_test
        (compile_match modenv repr partial) partial
        divide_constant
        (combine_constant arg cst partial)
        ctx pm
  | Pat_construct (cstr, _) ->
      compile_test
        (compile_match modenv repr partial) partial
        divide_constructor (combine_constructor modenv arg pat cstr partial)
        ctx pm
  | Pat_array _ ->
      let kind = Pmc_pattern.array_pattern_kind pat in
      compile_test (compile_match modenv repr partial) partial
        (divide_array kind) (combine_array arg kind partial)
        ctx pm
  | Pat_lazy _ ->
      assert false
  | Pat_variant(lab, _, row) ->
      compile_test (compile_match modenv repr partial) partial
        (divide_variant !row)
...
| PmVar {inside=pmh ; var_arg=arg} ->
    let lam, total =
      do_compile_matching modenv repr partial (ctx_lshift ctx) arg pmh in
    lam, jumps_map ctx_rshift total
| PmOr {body=body ; handlers=handlers} ->
    let lam, total = compile_match modenv repr partial ctx body in
    compile_orhandlers (compile_match modenv repr partial) lam total ctx handlers

and compile_no_test modenv divide up_ctx repr partial ctx to_match =
  let {pm=this_match ; ctx=this_ctx } = divide ctx to_match in
  let lambda,total = compile_match modenv repr partial this_ctx this_match in
...
let compile_matching modenv loc repr handler_fun arg pat_act_list partial =
  ...
  | Partial ->
      ...
      begin try
        let (lambda, total) = compile_match modenv repr partial (start_ctx 1) pm in
...
let partial_function modenv loc () =
  ...
  Lprim(Praise, [Lprim(Pmakeblock(0, Immutable),
          [Makelambda.transl_exception modenv Predef.cs_match_failure;
           Lconst(Const_block(0,
              [Const_base(Literal_string fname);
               Const_base(Literal_int line);
               Const_base(Literal_int char)]))])])

let for_function modenv loc repr param pat_act_list partial =
  compile_matching modenv loc repr (partial_function modenv loc) param pat_act_list partial

(* In the following two cases, exhaustiveness info is not available! *)
let for_trywith modenv param pat_act_list =
  compile_matching modenv Location.none None (fun () -> Lprim(Praise, [param]))
    param pat_act_list Partial

let for_let modenv loc param pat body =
  compile_matching modenv loc None (partial_function modenv loc) param [pat, body] Partial

(* Handling of tupled functions and matchings *)

(* Easy case since variables are available *)
let for_tupled_function modenv loc paraml pats_act_list partial =
  ...
  try
    let (lambda, total) = compile_match modenv None partial
        (start_ctx (List.length paraml)) pm in
    check_total total lambda raise_num (partial_function modenv loc)
  with
  | Unused -> partial_function modenv loc ()
...
| Pat_any ->  omegas size::k
| Pat_tuple args -> args::k
| Pat_or (p1,p2,_) ->  flatten_pat_line size p1 (flatten_pat_line size p2 k)
| Pat_alias (p,_) -> (* Note: if this 'as' pat is here, then this is a useless
                         binding, solves PR #3780 *)
    flatten_pat_line size p k
| _ -> Fatal.error "Matching.flatten_pat_line"

let flatten_cases size cases =
  List.map
    (fun (ps,action) -> match ps with
    | [p] -> flatten_pattern size p,action
    | _ -> Fatal.error "Matching.flatten_case")
    cases
...
let flatten_matrix size pss =
  List.fold_right
    ...
    | _   -> Fatal.error "Matching.flatten_matrix")
    pss []
...
let compile_flattened modenv repr partial ctx _ pmh = match pmh with
| Pm pm -> compile_match modenv repr partial ctx pm
| PmOr {body=b ; handlers=hs} ->
    let lam, total = compile_match modenv repr partial ctx b in
    compile_orhandlers (compile_match modenv repr partial) lam total ctx hs
| PmVar _ -> assert false

let do_for_multiple_match modenv loc paraml pat_act_list partial =
  let repr = None in
  let partial = check_partial pat_act_list partial in
  let raise_num,pm1 =
    match partial with
...
(* Once for checking that compilation is possible *)
      ...
      let lam, total =
        comp_match_handlers
          (compile_flattened modenv repr)
          partial (start_ctx size) () flat_next flat_nexts in
      List.fold_right2 (bind Strict) idl paraml
        (match partial with
        | Partial ->
            check_total total lam raise_num (partial_function modenv loc)
        | Total ->
            assert (jumps_is_empty total) ;
            lam)
    with Cannot_flatten ->
      let (lambda, total) = compile_match modenv None partial (start_ctx 1) pm1 in
  ...
  with Unused ->
    assert false (* ; partial_function modenv loc () *)
...
let for_multiple_match modenv loc paraml pat_act_list partial =
  let v_paraml = List.map param_to_var paraml in
  let paraml = List.map (fun (v,_) -> Lvar v) v_paraml in
  List.fold_right bind_opt v_paraml
    (do_for_multiple_match modenv loc paraml pat_act_list partial)
