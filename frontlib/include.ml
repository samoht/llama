...
let signatures modenv modname impl_sig intf_sig =
  let impl_env = Env.open_signature impl_sig (Env.create_empty modenv) in
...
let compunit modenv modname impl_name impl_sig intf_name intf_sig =
  try
    signatures modenv modname impl_sig intf_sig
  with Error reasons ->
