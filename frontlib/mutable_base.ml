...
let instantiate_region inst_r = function
  | Rparam p ->
      (try List.assq p inst_r
       with Not_found ->
         debug section "Cannot find region %d (inst_r=%s)"
           p (string_of_inst string_of_mutable_region inst_r);
         raise Not_found)
  | Rconstr rcs ->
      let r = new_mutable_region () in
      r.rbody <- MRconstr rcs;
      r
