...
let comparisons_table = create_hashtable 11 [
  "%equal",
      (Pccall{prim_name = "llama_equal"; prim_arity = 2; prim_alloc = true;
              prim_native_name = ""; prim_native_float = false},
       Pintcomp Ceq,
       Pfloatcomp Ceq,
       Pccall{prim_name = "llama_string_equal"; prim_arity = 2;
              prim_alloc = false;
              prim_native_name = ""; prim_native_float = false},
       ...
       true);
  "%notequal",
      (Pccall{prim_name = "llama_notequal"; prim_arity = 2; prim_alloc = true;
              prim_native_name = ""; prim_native_float = false},
       Pintcomp Cneq,
       Pfloatcomp Cneq,
       Pccall{prim_name = "llama_string_notequal"; prim_arity = 2;
       ...
       true);
  "%lessthan",
      (Pccall{prim_name = "llama_lessthan"; prim_arity = 2; prim_alloc = true;
              prim_native_name = ""; prim_native_float = false},
       Pintcomp Clt,
       Pfloatcomp Clt,
       Pccall{prim_name = "llama_string_lessthan"; prim_arity = 2;
              prim_alloc = false; prim_native_name = "";
              prim_native_float = false},
       ...
       false);
  "%greaterthan",
      (Pccall{prim_name = "llama_greaterthan"; prim_arity = 2; prim_alloc = true;
              prim_native_name = ""; prim_native_float = false},
       Pintcomp Cgt,
       Pfloatcomp Cgt,
       Pccall{prim_name = "llama_string_greaterthan"; prim_arity = 2;
              prim_alloc = false; prim_native_name = "";
              prim_native_float = false},
       ...
       false);
  "%lessequal",
      (Pccall{prim_name = "llama_lessequal"; prim_arity = 2; prim_alloc = true;
              prim_native_name = ""; prim_native_float = false},
       Pintcomp Cle,
       Pfloatcomp Cle,
       Pccall{prim_name = "llama_string_lessequal"; prim_arity = 2;
              prim_alloc = false; prim_native_name = "";
              prim_native_float = false},
       ...
       false);
  "%greaterequal",
      (Pccall{prim_name = "llama_greaterequal"; prim_arity = 2;
              prim_alloc = true;
              prim_native_name = ""; prim_native_float = false},
       Pintcomp Cge,
       Pfloatcomp Cge,
       Pccall{prim_name = "llama_string_greaterequal"; prim_arity = 2;
              prim_alloc = false; prim_native_name = "";
              prim_native_float = false},
       ...
       false);
  "%compare",
      (Pccall{prim_name = "llama_compare"; prim_arity = 2; prim_alloc = true;
              prim_native_name = ""; prim_native_float = false},
       Pccall{prim_name = "llama_int_compare"; prim_arity = 2;
              prim_alloc = false; prim_native_name = "";
              prim_native_float = false},
       Pccall{prim_name = "llama_float_compare"; prim_arity = 2;
              prim_alloc = false; prim_native_name = "";
              prim_native_float = false},
       Pccall{prim_name = "llama_string_compare"; prim_arity = 2;
              prim_alloc = false; prim_native_name = "";
              prim_native_float = false},
       Pccall{prim_name = "llama_nativeint_compare"; prim_arity = 2;
              prim_alloc = false; prim_native_name = "";
              prim_native_float = false},
       Pccall{prim_name = "llama_int32_compare"; prim_arity = 2;
              prim_alloc = false; prim_native_name = "";
              prim_native_float = false},
       Pccall{prim_name = "llama_int64_compare"; prim_arity = 2;
              prim_alloc = false; prim_native_name = "";
              prim_native_float = false},
       false)
]
...
let extract_float = function
  ...
  | _ -> Fatal.error "Translcore.extract_float"
...
let assert_failed modenv loc =
  ...
  Lprim(Praise, [Lprim(Pmakeblock(0, Immutable),
          [Makelambda.transl_exception modenv Predef.cs_assert_failure;
...
let rec transl_exp modenv e =
  match e.exp_desc with
    Exp_var var ->
      Lvar (Makeident.of_variable var)
  | Exp_value v ->
      begin match v with
        | {val_kind = Val_prim p} ->
            transl_primitive p
        | {val_kind = Val_reg} ->
            Makelambda.transl_value modenv v
      end
  | Exp_literal lit ->
      Lconst (Const_base lit)
  | Exp_let(rec_flag, pat_expr_list, body) ->
      transl_let modenv rec_flag pat_expr_list (event_before body (transl_exp modenv body))
  | Exp_function pat_expr_list ->
      let ((kind, params), body) =
        event_function e
          (function repr ->
            transl_function modenv e.exp_loc !Frontconfig.native_code repr pat_expr_list)
      in
  ...
  | Exp_apply({exp_desc = Exp_value v}, oargs)
      ...
      let wrap f =
        ...
        else event_after e (transl_apply modenv f args' e.exp_loc)
      in
      let wrap0 f =
        if args' = [] then f else wrap f in
      let argl = transl_list modenv args in
      let public_send = p.prim_name = "%send"
        || not !Frontconfig.native_code && p.prim_name = "%sendcache"in
  ...
  | Exp_apply(funct, oargs) ->
      event_after e (transl_apply modenv (transl_exp modenv funct) oargs e.exp_loc)
  | Exp_match({exp_desc = Exp_tuple argl}, pat_expr_list) ->
      ...
      Matching.for_multiple_match modenv e.exp_loc
        (transl_list modenv argl) (transl_cases modenv pat_expr_list) partial
  | Exp_match(arg, pat_expr_list) ->
      let pat_expr_list = Pmc_pattern.import_cases pat_expr_list in
      let partial = Parmatch.check_partial e.exp_loc pat_expr_list in
      Parmatch.check_unused pat_expr_list;
      Matching.for_function modenv e.exp_loc None
        (transl_exp modenv arg) (transl_cases modenv pat_expr_list) partial
  | Exp_try(body, pat_expr_list) ->
      let pat_expr_list = Pmc_pattern.import_cases pat_expr_list in
      Parmatch.check_unused pat_expr_list;
      let id = Ident.create (name_pattern "exn" pat_expr_list) in
      Ltrywith(transl_exp modenv body, id,
               Matching.for_trywith modenv (Lvar id) (transl_cases modenv pat_expr_list))
  | Exp_tuple el ->
      let ll = transl_list modenv el in
      begin try
  ...
  | Exp_construct(cstr, args) ->
      let ll = transl_list modenv args in
      ...
      | Tag_exception ->
          Lprim(Pmakeblock(0, Immutable), Makelambda.transl_exception modenv cstr :: ll)
      end
  | Exp_record (tcs, lbl_expr_list, opt_init_expr) ->
      transl_record modenv (get_labels tcs) (*lbl1.lbl_repres*)Record_regular lbl_expr_list opt_init_expr
  | Exp_field(arg, lbl) ->
...
*)
      Lprim(access, [transl_exp modenv arg])
...
*)
      Lprim(access, [transl_exp modenv arg; transl_exp modenv newval])
  | Exp_array expr_list ->
      let kind = array_kind e in
      let ll = transl_list modenv expr_list in
      begin try
  ...
  | Exp_ifthenelse(cond, ifso, Some ifnot) ->
      Lifthenelse(transl_exp modenv cond,
                  event_before ifso (transl_exp modenv ifso),
                  event_before ifnot (transl_exp modenv ifnot))
  | Exp_ifthenelse(cond, ifso, None) ->
      Lifthenelse(transl_exp modenv cond,
                  event_before ifso (transl_exp modenv ifso),
                  lambda_unit)
  | Exp_sequence(expr1, expr2) ->
      Lsequence(transl_exp modenv expr1, event_before expr2 (transl_exp modenv expr2))
  | Exp_while(cond, body) ->
      Lwhile(transl_exp modenv cond, event_before body (transl_exp modenv body))
  | Exp_for(param, low, high, dir, body) ->
      Lfor(Makeident.of_variable param, transl_exp modenv low, transl_exp modenv high, dir,
           event_before body (transl_exp modenv body))
  | Exp_when(cond, body) ->
      event_before cond
        (Lifthenelse(transl_exp modenv cond, event_before body (transl_exp modenv body),
                     staticfail))
  | Exp_assert (cond) ->
      if !Frontconfig.noassert
      then lambda_unit
...
and transl_cases modenv pat_expr_list =
  List.map
    (fun (pat, expr) -> (pat, event_before expr (transl_exp modenv expr)))
    pat_expr_list

and transl_tupled_cases modenv patl_expr_list =
  List.map (fun (patl, expr) -> (patl, transl_exp modenv expr)) patl_expr_list

and transl_apply modenv lam sargs loc =
  ...
  in
  lapply lam (List.map (transl_exp modenv) sargs)

and transl_function modenv loc untuplify_fn repr pat_expr_list =
  let pat_expr_list = Pmc_pattern.import_cases pat_expr_list in
  ...
  match pat_expr_list with
    ...
    when Parmatch.fluid pat ->
      ...
      let ((_, params), body) =
        transl_function modenv exp.exp_loc false repr pl in
      ((Curried, param :: params),
       Matching.for_function modenv loc None (Lvar param) [pat, body] partial)
  | ({Pmc_pattern.pat_desc = Pmc_pattern.Pat_tuple pl}, _) :: _ when untuplify_fn ->
      begin try
        ...
        ((Tupled, params),
         Matching.for_tupled_function modenv loc params
           (transl_tupled_cases modenv pats_expr_list) partial)
      with Matching.Cannot_flatten ->
        let param = Ident.create(name_pattern "param" pat_expr_list) in
        ((Curried, [param]),
         Matching.for_function modenv loc repr (Lvar param)
           (transl_cases modenv pat_expr_list) partial)
      end
  | _ ->
      let param = Ident.create(name_pattern "param" pat_expr_list) in
      ((Curried, [param]),
...
and transl_let modenv rec_flag pat_expr_list body =
  match rec_flag with
    Nonrecursive (*| Default*) ->
      ...
      | (pat, expr) :: rem ->
          ...
          Matching.for_let modenv loc (transl_exp modenv expr) pat (transl rem)
        ...
        List.map
          (fun (pat, expr) ->
            match pat.pat_desc with
              Pat_var id -> Makeident.of_variable id
            | _ -> raise(Error(pat.pat_loc, Illegal_letrec_pat)))
        pat_expr_list in
      let transl_case (pat, expr) id =
        let lam = transl_exp modenv expr in
        if not (check_recursive_lambda idlist lam) then
          raise(Error(expr.exp_loc, Illegal_letrec_expr));
        (id, lam) in
      Lletrec(List.map2 transl_case pat_expr_list idlist, body)

and transl_record modenv all_labels repres lbl_expr_list opt_init_expr =
...
*)
    ...
    List.iter
      (fun (lbl, expr) -> lv.(lbl.lbl_pos) <- transl_exp modenv expr)
      lbl_expr_list;
    ...
    begin match opt_init_expr with
      None -> lam
    | Some init_expr -> Llet(Strict, init_id, transl_exp modenv init_expr, lam)
    end
  end else begin
...
(*
        ...
        | Record_float -> Psetfloatfield lbl.lbl_pos in
*)
      Lsequence(Lprim(upd, [Lvar copy_id; transl_exp modenv expr]), cont) in
    begin match opt_init_expr with
      None -> assert false
    | Some init_expr ->
        Llet(Strict, copy_id,
             Lprim(Pduprecord (repres, size), [transl_exp modenv init_expr]),
