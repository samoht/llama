...
let report_error ppf = function
  | Unclosed(opening_loc, opening, closing_loc, closing) ->
      if String.length !Frontlocation.input_name = 0
      && Frontlocation.highlight_locations ppf opening_loc closing_loc
      then fprintf ppf "Syntax error: '%s' expected, \
        ...
        fprintf ppf "%aSyntax error: '%s' expected@."
          Frontlocation.print_error closing_loc closing;
        fprintf ppf "%aThis '%s' might be unmatched"
          Frontlocation.print_error opening_loc opening
      end
  | Applicative_path loc ->
      fprintf ppf "%aSyntax error: applicative paths of the form F(X).t are not supported when the option -no-app-func is set."
        Frontlocation.print_error loc
  | Other loc ->
      fprintf ppf "%aSyntax error" Frontlocation.print_error loc
