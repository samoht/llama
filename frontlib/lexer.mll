...
rule token = parse
  ...
  | "(*)"
      { let loc = Location.curr lexbuf in
        Frontlocation.prerr_warning loc Warnings.Comment_start;
        comment_start_loc := [Location.curr lexbuf];
        comment lexbuf;
        token lexbuf
      }
  | "*)"
      { let loc = Location.curr lexbuf in
        Frontlocation.prerr_warning loc Warnings.Comment_not_end;
...
*)
          let loc = Location.curr lexbuf in
          Frontlocation.prerr_warning loc Warnings.Illegal_backslash;
  ...
  | newline
      { if not (in_comment ()) then
          Frontlocation.prerr_warning (Location.curr lexbuf) Warnings.Eol_in_string;
        update_loc lexbuf None 1 false 0;
        let s = Lexing.lexeme lexbuf in
