...
(** {6 File operations on large files} *)
    ...
    type stats64 =
      { st64_dev : int;               (** Device number *)
        st64_ino : int;               (** Inode number *)
        st64_kind : file_kind;        (** Kind of the file *)
        st64_perm : file_perm;        (** Access rights *)
        st64_nlink : int;             (** Number of links *)
        st64_uid : int;               (** User id of the owner *)
        st64_gid : int;               (** Group ID of the file's group *)
        st64_rdev : int;              (** Device minor number *)
        st64_size : int64;            (** Size in bytes *)
        st64_atime : float;           (** Last access time *)
        st64_mtime : float;           (** Last modification time *)
        st64_ctime : float;           (** Last status change time *)
      }
