...
let engine tbl state buf =
  ...
  if result >= 0 then begin
    ...
    buf.lex_curr_p <- {pos_fname = buf.lex_curr_p.pos_fname;
                       pos_lnum  = buf.lex_curr_p.pos_lnum;
                       pos_bol   = buf.lex_curr_p.pos_bol;
...
let lex_refill read_fun aux_buffer lexbuf =
  ...
  *)
  if lexbuf.lex_buffer_len + n > String.length lexbuf.lex_buffer then begin
    ...
    then begin
      (* But there is enough space if we reclaim the junk at the beginning
         of the buffer *)
      String.blit_string lexbuf.lex_buffer lexbuf.lex_start_pos
                  lexbuf.lex_buffer 0
      ...
      let newbuf = String.create_string newlen in
  ...
  String.blit_string aux_buffer 0
...
let from_function f =
  { refill_buff = lex_refill f (String.create_string 512);
    lex_buffer = String.create_string 1024;
    lex_buffer_len = 0;
    lex_abs_pos = 0;
    lex_start_pos = 0;
    lex_curr_pos = 0;
...
let lexeme lexbuf =
  ...
  let s = String.create_string len in
...
let new_line lexbuf =
  ...
  lexbuf.lex_curr_p <- { pos_fname = lcp.pos_fname;
...
let flush_input lb =
  ...
  lb.lex_abs_pos <- 0;
  lb.lex_curr_p <- {
    pos_fname = lb.lex_curr_p.pos_fname;
    pos_lnum  = lb.lex_curr_p.pos_lnum;
    pos_bol   = lb.lex_curr_p.pos_bol;
    (* with *) pos_cnum = 0
  };
  lb.lex_buffer_len <- 0;
;;
