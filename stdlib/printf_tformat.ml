...
let bad_conversion_format fmt i c =
  bad_conversion (Printf_sformat.to_string fmt) i c
;;
...
let extract_format fmt start stop widths =
  let skip_positional_spec start =
    match Printf_sformat.unsafe_get fmt start with
    | '0'..'9' ->
  ...
  let rec fill_format i widths =
    if i <= stop then
      match (Printf_sformat.unsafe_get fmt i, widths) with
      | ('*', h :: t) ->
 ...
 (* | '$' -> scan_flags skip (succ i) *** PR#4321 *)
  ...
  and scan_conv skip i =
    ...
    | '{' as conv ->
      ...
      let rec loop i =
        if i < j - 2 then loop (add_char i (Printf_sformat.get fmt i)) in
      loop i;
...
let kapr kpr fmt =
  ...
  | 0 -> kpr fmt [||]
  | 1 -> Obj.magic (fun x ->
      let a = Array.create 1 (Obj.repr 0) in
      a.(0) <- x;
      kpr fmt a)
  | 2 -> Obj.magic (fun x y ->
      let a = Array.create 2 (Obj.repr 0) in
      a.(0) <- x; a.(1) <- y;
      kpr fmt a)
  | 3 -> Obj.magic (fun x y z ->
      let a = Array.create 3 (Obj.repr 0) in
      a.(0) <- x; a.(1) <- y; a.(2) <- z;
      kpr fmt a)
  | 4 -> Obj.magic (fun x y z t ->
      let a = Array.create 4 (Obj.repr 0) in
      a.(0) <- x; a.(1) <- y; a.(2) <- z;
      a.(3) <- t;
      kpr fmt a)
  | 5 -> Obj.magic (fun x y z t u ->
      let a = Array.create 5 (Obj.repr 0) in
      a.(0) <- x; a.(1) <- y; a.(2) <- z;
      a.(3) <- t; a.(4) <- u;
      kpr fmt a)
  | 6 -> Obj.magic (fun x y z t u v ->
      let a = Array.create 6 (Obj.repr 0) in
      a.(0) <- x; a.(1) <- y; a.(2) <- z;
      a.(3) <- t; a.(4) <- u; a.(5) <- v;
      kpr fmt a)
  | nargs ->
    let rec loop i args =
      if i >= nargs then
        let a = Array.create nargs (Obj.repr 0) in
...
type positional_specification =
   | Spec_none | Spec_index of Printf_sformat.index
;;
...
let scan_positional_spec fmt got_spec n i =
  ...
  | '0'..'9' as d ->
    let rec get_int_literal accu j =
      match Printf_sformat.unsafe_get fmt j with
      | '0'..'9' as d ->
        get_int_literal (10 * accu + (int_of_char d - 48)) (succ j)
      | '$' ->
        if accu = 0 then
          failwith "printf: bad positional specification (0)." else
        got_spec (Spec_index (Printf_sformat.index_of_literal_position accu)) (succ j)
...
let next_index spec n =
  match spec with
  | Spec_none -> Printf_sformat.succ_index n
  | Spec_index _ -> n
;;
...
(* Note: here, rather than test explicitly against [Printf_sformat.length fmt]
   to detect the end of the format, we use [Printf_sformat.unsafe_get] and
...
let mkprintf to_s get_out outc outs flush k fmt =
  ...
  let rec pr k n fmt v =
    ...
    and cont_f n i =
      flush out; doprn n i
    and cont_m n xf i =
      let m = Printf_sformat.add_int_index (count_arguments_of_format xf) n in
      pr (Obj.magic (fun _ -> doprn m i)) n xf v in

    doprn n 0 in

  let kpr = pr k (Printf_sformat.index_of_int 0) in
...
let get_buff fmt =
  let len = 2 * Printf_sformat.length fmt in
  Buffer.create len
;;
