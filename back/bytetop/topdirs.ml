...
let check_consistency crcs ppf filename cu =
  try
    List.iter
      (fun (name, crc) -> Consistbl.check crcs name crc filename)
      cu.cu_imports
...
let load_compunit crcs ic filename ppf compunit =
  check_consistency crcs ppf filename compunit;
  ...
  try
    ...
    let success = try
      if buffer = Config.cmo_magic_number then begin
        ...
        load_compunit crcs ic filename ppf (input_value ic : compilation_unit);
        true
      end else
        ...
        List.iter
          (fun dllib ->
            ...
            with Failure reason ->
              ...
              raise Load_failed)
          lib.lib_dllibs;
        List.iter (load_compunit crcs ic filename ppf) lib.lib_units;
        true
      end else begin
...
let dir_load ppf env name = ignore (load_file (Modenv.loaded_crcs (Env.modenv env)) ppf name)

let _ = Hashtbl.add directive_table "load" (Directive_string (dir_load std_out))

(* Load commands from a file *)

let dir_use ppf env name = ignore(Toploop.use_file env ppf name)
...
let match_printer_type ppf env desc typename =
  let printer_tcs =
    try
      Env.lookup_type_constructor (Ldot("Topdirs", typename)) env
    with Not_found ->
...
let find_printer_type ppf env lid =
  try
    let v = Env.lookup_value lid env in
    let (ty_arg, is_old_style) =
      try
        (match_printer_type ppf env v "printer_type_new", false)
      with Mutable_base.Unify _ ->
        (match_printer_type ppf env v "printer_type_old", true) in
...
let dir_install_printer ppf env lid =
  try
    let (ty_arg, v, is_old_style) = find_printer_type ppf env lid in
    let o = eval_value (Env.modenv env) v in
    let print_function =
...
let dir_remove_printer ppf env lid =
  try
    let (ty_arg, path, is_old_style) = find_printer_type ppf env lid in
    begin try
      remove_printer path
    with Not_found ->
...
external current_environment: unit -> Obj.t = "llama_get_current_environment"

let tracing_function_ptr =
  get_code_pointer
    (Obj.repr (fun arg -> print_trace (current_environment()) arg))

let dir_trace ppf env lid =
  try
    let v = Env.lookup_value lid env in
    ...
    | Val_prim p ->
        ...
        Printtyp.longident lid
    | _ ->
        let clos = Toploop.eval_value (Env.modenv env) v in
        ...
        | None ->
            ...
            traced_functions :=
              { path = v;
                ...
                instrumented_fun =
                  instrument_closure env lid ppf v.val_type }
              :: !traced_functions;
...
let parse_warnings ppf iserr _env s =
...
(* Control the printing of values *)
  ...
  Hashtbl.add directive_table "print_depth"
             (Directive_int(fun _env n -> max_printer_depth := n));
  Hashtbl.add directive_table "print_length"
             (Directive_int(fun _env n -> max_printer_steps := n));
