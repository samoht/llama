...
val loop : Consistbl.t -> formatter -> unit

(* Read and execute a script from the given file *)

val run_script : Consistbl.t -> formatter -> string -> string array -> Env.t option
...
val execute_phrase : Env.t -> bool -> formatter -> Parsetree.toplevel_phrase -> Env.t option
...
val use_file : Env.t -> formatter -> string -> Env.t option
val use_silently : Env.t -> formatter -> string -> Env.t option
        (* Read and execute commands from a file.
           [use_file] prints the types and values of the results.
           [use_silently] does not print them. *)
val eval_exception: Modenv.t -> Base.constructor -> Obj.t
val eval_value : Modenv.t -> Base.value -> Obj.t
        (* Return the toplevel object referred to by the given path *)

(* Printing of values *)

val print_value: Env.t -> Obj.t -> formatter -> Base.llama_type -> unit
val print_untyped_exception: formatter -> Obj.t -> unit

val install_printer :
  Base.value -> Base.llama_type -> (formatter -> Obj.t -> unit) -> unit
val remove_printer : Base.value -> unit
