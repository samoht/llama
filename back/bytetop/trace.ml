...
let rec instrument_result env name ppf clos_typ =
  match Basics.expand_type clos_typ with
  | Tarrow(t1, t2, _) ->
      let starred_name =
        match name with
        | Lident s -> Lident(s ^ "*")
          ...
          if not !may_trace then
            ...
            with exn ->
              ...
              fprintf ppf "@[<2>%a raises@ %a@]@."
                Printtyp.longident starred_name
                (print_value env (Obj.repr exn)) Predef.dummy_type_exn;
              may_trace := true;
              raise exn
          end))
  | _ -> (fun v -> v)
...
let rec find_traced_closure clos = function
  | [] -> Fatal.error "Trace.find_traced_closure"
