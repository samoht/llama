...
/* Parse the LLAMA_STDLIB_DIR/ld.conf file and add the directories
...
static char * parse_ld_conf(void)
{
  ...
  stdlib = getenv("LLAMALIB");
  if (stdlib == NULL) stdlib = LLAMA_STDLIB_DIR;
                                ...
                                char * req_prims)
{
  ...
  /* Initialize the search path for dynamic libraries:
     ...
     - directories specified in the LLAMA_LD_LIBRARY_PATH
