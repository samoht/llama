...
static void check_head (value v)
{
  ...
  Assert (Color_hd (Hd_val (v)) != Llama_blue);
  Assert (Is_in_heap (v));
  ...
  while (chunk != NULL){
    ...
    while (cur_hp < chunk_end){
      ...
      case Llama_white:
        ...
        break;
      case Llama_gray: case Llama_black:
      ...
      case Llama_blue:
        ...
        Assert (prev_hp == NULL
                || (Color_hp (prev_hp) != Llama_blue && Wosize_hp (prev_hp) > 0)
                || cur_hp == llama_gc_sweep_hp);
        Assert (Next (cur_hp) == chunk_end
                || (Color_hp (Next (cur_hp)) != Llama_blue
