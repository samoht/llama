...
extern void * llama_dlopen(char * libname, int for_execution, int global);

/* Close a shared library handle */
extern void llama_dlclose(void * handle);

/* Look up the given symbol in the given shared library.
   Return [NULL] if not found, or symbol value if found. */
extern void * llama_dlsym(void * handle, char * name);

extern void * llama_globalsym(char * name);

/* Return an error message describing the most recent dynlink failure. */
extern char * llama_dlerror(void);

/* Add to [contents] the (short) names of the files contained in
   the directory named [dirname].  No entries are added for [.] and [..].
   Return 0 on success, -1 on error; set errno in the case of error. */
extern int llama_read_directory(char * dirname, struct ext_table * contents);

#ifdef __linux__
/* Recover executable name from /proc/self/exe if possible */
extern int llama_executable_name(char * name, int name_len);
#endif

#endif /* LLAMA_OSDEPS_H */
