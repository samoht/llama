...
} sentinel = {0, Make_header (0, 0, Llama_blue), 0, 0};
...
static char *allocate_block (mlsize_t wh_sz, int flpi, char *prev, char *cur)
{
  ...
  if (Wosize_hd (h) < wh_sz + 1){                        /* Cases 0 and 1. */
    ...
    Hd_op (cur) = Make_header (0, 0, Llama_white);
    ...
    Hd_op (cur) = Make_header (Wosize_hd (h) - wh_sz, 0, Llama_blue);
  }
...
char *llama_fl_merge_block (char *bp)
{
  ...
  if (last_fragment == Hp_bp (bp)){
    ...
    if (bp_whsz <= Max_wosize){
      hd = Make_header (bp_whsz, 0, Llama_white);
      bp = last_fragment;
      Hd_bp (bp) = hd;
  ...
  if (adj == Hp_bp (cur)){
    ...
    if (Wosize_hd (hd) + cur_whsz <= Max_wosize){
      ...
      hd = Make_header (Wosize_hd (hd) + cur_whsz, 0, Llama_blue);
      Hd_bp (bp) = hd;
      adj = bp + Bosize_hd (hd);
#ifdef DEBUG
      fl_last = NULL;
  ...
  if (prev + Bsize_wsize (prev_wosz) == Hp_bp (bp)
    ...
    Hd_bp (prev) = Make_header (prev_wosz + Whsize_hd (hd), 0,Llama_blue);
#ifdef DEBUG
...
void llama_make_free_blocks (value *p, mlsize_t size, int do_merge)
{
  ...
  while (size > 0){
    ...
    *(header_t *)p = Make_header (Wosize_whsize (sz), 0, Llama_white);
