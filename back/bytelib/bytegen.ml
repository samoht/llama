...
let merge_infos ev ev' =
  ...
  | _                 -> Fatal.error "Bytegen.merge_infos"
...
let merge_repr ev ev' =
  ...
  | _, _          -> Fatal.error "Bytegen.merge_repr"
...
let comp_bint_primitive bi suff args =
  let pref =
    match bi with Pnativeint -> "llama_nativeint_"
                | Pint32 -> "llama_int32_"
                | Pint64 -> "llama_int64_" in
...
let comp_primitive p args =
  ...
  | Pduprecord _ -> Kccall("llama_obj_dup", 1)
  ...
  | Pintoffloat -> Kccall("llama_int_of_float", 1)
  | Pfloatofint -> Kccall("llama_float_of_int", 1)
  | Pnegfloat -> Kccall("llama_neg_float", 1)
  | Pabsfloat -> Kccall("llama_abs_float", 1)
  | Paddfloat -> Kccall("llama_add_float", 2)
  | Psubfloat -> Kccall("llama_sub_float", 2)
  | Pmulfloat -> Kccall("llama_mul_float", 2)
  | Pdivfloat -> Kccall("llama_div_float", 2)
  | Pfloatcomp Ceq -> Kccall("llama_eq_float", 2)
  | Pfloatcomp Cneq -> Kccall("llama_neq_float", 2)
  | Pfloatcomp Clt -> Kccall("llama_lt_float", 2)
  | Pfloatcomp Cgt -> Kccall("llama_gt_float", 2)
  | Pfloatcomp Cle -> Kccall("llama_le_float", 2)
  | Pfloatcomp Cge -> Kccall("llama_ge_float", 2)
  | Pstringlength -> Kccall("llama_ml_string_length", 1)
  | Pstringrefs -> Kccall("llama_string_get", 2)
  | Pstringsets -> Kccall("llama_string_set", 3)
  | Pstringrefu -> Kgetstringchar
  | Pstringsetu -> Ksetstringchar
  | Parraylength kind -> Kvectlength
  | Parrayrefs Pgenarray -> Kccall("llama_array_get", 2)
  | Parrayrefs Pfloatarray -> Kccall("llama_array_get_float", 2)
  | Parrayrefs _ -> Kccall("llama_array_get_addr", 2)
  | Parraysets Pgenarray -> Kccall("llama_array_set", 3)
  | Parraysets Pfloatarray -> Kccall("llama_array_set_float", 3)
  | Parraysets _ -> Kccall("llama_array_set_addr", 3)
  | Parrayrefu Pgenarray -> Kccall("llama_array_unsafe_get", 2)
  | Parrayrefu Pfloatarray -> Kccall("llama_array_unsafe_get_float", 2)
  | Parrayrefu _ -> Kgetvectitem
  | Parraysetu Pgenarray -> Kccall("llama_array_unsafe_set", 3)
  | Parraysetu Pfloatarray -> Kccall("llama_array_unsafe_set_float", 3)
  | Parraysetu _ -> Ksetvectitem
  | Pisint -> Kisint
  | Pisout -> Kisout
  | Pbittest -> Kccall("llama_bitvect_test", 2)
  | Pbintofint bi -> comp_bint_primitive bi "of_int" args
  | Pintofbint bi -> comp_bint_primitive bi "to_int" args
  | Pcvtbint(Pint32, Pnativeint) -> Kccall("llama_nativeint_of_int32", 1)
  | Pcvtbint(Pnativeint, Pint32) -> Kccall("llama_nativeint_to_int32", 1)
  | Pcvtbint(Pint32, Pint64) -> Kccall("llama_int64_of_int32", 1)
  | Pcvtbint(Pint64, Pint32) -> Kccall("llama_int64_to_int32", 1)
  | Pcvtbint(Pnativeint, Pint64) -> Kccall("llama_int64_of_nativeint", 1)
  | Pcvtbint(Pint64, Pnativeint) -> Kccall("llama_int64_to_nativeint", 1)
  ...
  | Pbintcomp(bi, Ceq) -> Kccall("llama_equal", 2)
  | Pbintcomp(bi, Cneq) -> Kccall("llama_notequal", 2)
  | Pbintcomp(bi, Clt) -> Kccall("llama_lessthan", 2)
  | Pbintcomp(bi, Cgt) -> Kccall("llama_greaterthan", 2)
  | Pbintcomp(bi, Cle) -> Kccall("llama_lessequal", 2)
  | Pbintcomp(bi, Cge) -> Kccall("llama_greaterequal", 2)
  | Pbigarrayref(_, n, _, _) -> Kccall("llama_ba_get_" ^ string_of_int n, n + 1)
  | Pbigarrayset(_, n, _, _) -> Kccall("llama_ba_set_" ^ string_of_int n, n + 2)
  | _ -> Fatal.error "Bytegen.comp_primitive"
  ...
  match exp with
    Lvar id ->
      ...
      with Not_found ->
        ...
        Fatal.error ("Bytegen.comp_expr: var " ^ Ident.unique_name id)
      end
  | Lconst cst ->
      Kconst cst :: cont
      ...
      if List.for_all (function (_, Lfunction(_,_,_)) -> true | _ -> false)
        ...
        let rec comp_init new_env sz = function
          ...
          | (id, exp, RHS_floatblock blocksize) :: rem ->
              ...
              Kccall("llama_alloc_dummy_float", 1) :: Kpush ::
              comp_init (add_var id (sz+1) new_env) (sz+1) rem
          | (id, exp, RHS_block blocksize) :: rem ->
              Kconst(Const_base(Literal_int blocksize)) ::
              Kccall("llama_alloc_dummy", 1) :: Kpush ::
              comp_init (add_var id (sz+1) new_env) (sz+1) rem
        ...
        and comp_rec new_env sz i = function
          ...
          | (id, exp, (RHS_block _ | RHS_floatblock _)) :: rem ->
              comp_expr new_env exp sz
                (Kpush :: Kacc i :: Kccall("llama_update_dummy", 2) ::
      ...
      | Pgenarray ->
          ...
          then Kmakeblock(0, 0) :: cont
                 ...
                 (Kmakeblock(List.length args, 0) ::
                  Kccall("llama_make_array", 1) :: cont)
      end
...
(* Build label vectors *)
  ...
  | Lassign(id, expr) ->
      ...
      with Not_found ->
        Fatal.error "Bytegen.comp_expr: assign"
      end
  | Levent(lam, lev) ->
      let event kind info =
        { ev_pos = 0;                   (* patched in emitcode *)
      ...
      | Lev_after ->
          let info =
            ...
            | _                 -> Event_other
          in
          let ev = event Event_after info in
...
let comp_block env exp sz cont =
  ...
  if !max_stack_used + 1 > Config.stack_threshold then
    ...
    Kccall("llama_ensure_stack_capacity", 1) ::
    code
  else
    code
