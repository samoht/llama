...
let out_const c =
  ...
  with
  | AsInt -> Fatal.error "Emitcode.const_as_int"
...
let define_label lbl =
  ...
  match (!label_table).(lbl) with
    Label_defined _ ->
      Fatal.error "Emitcode.define_label"
...
let to_file outchan unit_name crcs primitives code =
  init();
  ...
  let compunit =
    { cu_name = unit_name;
      ...
      cu_imports = crcs;
      cu_primitives = List.map Primitive.byte_name primitives;
      cu_force_link = false;
      cu_debug = pos_debug;
      cu_debugsize = size_debug } in
  init();                               (* Free out_buffer and reloc_info *)
(*  Typeutil.cleanup_abbrev ();              (* Remove any cached abbreviation
