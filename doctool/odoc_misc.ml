...
let string_of_info i =
  (match i.Odoc_types.i_deprecated with
    None -> ""
  | Some d -> Odoc_messages.deprecated^"! "^(string_of_text d)^"\n")^
  (match i.Odoc_types.i_desc with
    None -> ""
  | Some d when d = [Odoc_types.Raw ""] -> ""
  | Some d -> (string_of_text d)^"\n"
  )^
  (string_of_author_list i.Odoc_types.i_authors)^
  (string_of_version_opt i.Odoc_types.i_version)^
  (string_of_since_opt i.Odoc_types.i_since)^
  (string_of_raised_exceptions i.Odoc_types.i_raised_exceptions)^
  (string_of_return_opt i.Odoc_types.i_return_value)
