...
let rec expr env ex =
  { exp_desc =
      begin match ex.pexp_desc with
        ...
        | Pexp_let (b, lpe, e) ->
            let big_env =
              List.fold_right (fun (p,e) env -> extend_env env p) lpe env
            in
            let cond_env = if b then big_env else env in
            Texp_let (b, List.map(fun (p,e) -> pattern env p, expr cond_env e) lpe, expr big_env e)
        | Pexp_function l ->
            Texp_function (List.map (fun (lp,e) -> List.map (pattern env) lp,
                                       expr (List.fold_left extend_env env lp) e) l)
        | Pexp_try (e, lpe) ->
            Texp_try (expr env e, List.map (fun (p,e) -> pattern env p,expr (extend_env env p) e) lpe)
        ...
        | Pexp_for(s,e1,e2,b,e3) -> Texp_for(Id.create s,expr env e1,expr env e2,b,expr (ext env s) e3)
            ...
            let rec aux env l e =
              match l with
                ...
                | sp::rest ->
                    let sp, env = 
                    begin match sp with
                      | Ptermpat p -> Ztermpat (pattern env p),(extend_env env p)
                      | Pnontermpat (e, p) ->
                          Znontermpat (expr env e, pattern env p), (extend_env env p)
                      | Pexp_streampat s ->          Texp_streampat (Id.create s) , (ext env s)
                    end
                    in
