...
let compile_implementation modname filename suffix =
  if Sys.file_exists (filename ^ ".mli") then begin
    try
      let intfname =
        try
          find_in_path !Config.load_path (String.lowercase modname ^ ".cmi")
        with Not_found ->
          eprintf
            "Cannot find file %s.cmi. Please compile %s.mli first.\n"
            modname filename;
          raise Toplevel in
