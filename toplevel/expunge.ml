...
let expunge_map tbl =
  Symtable.filter_global_map
    (fun id -> Set.mem (Ident.name id) !to_keep)
    tbl

let expunge_crcs tbl =
  List.filter (fun (unit, crc) -> Set.mem unit !to_keep) tbl

let main () =
  let input_name = Sys.argv.(1) in
  let output_name = Sys.argv.(2) in
  Array.iter
    (fun exn -> to_keep := Set.add exn !to_keep)
    Runtimedef.builtin_exceptions;
  for i = 3 to Array.length Sys.argv - 1 do
    to_keep := Set.add (String.capitalize Sys.argv.(i)) !to_keep
  done;
