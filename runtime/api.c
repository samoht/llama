...
void llama_realloc_globals(size_t sz) {
...
char* llama_stat_alloc(size_t n) {
    return(stat_alloc(n));
}

void llama_stat_free(char* v) {
    stat_free(v);
}

char* llama_stat_resize(char* v, size_t n) {
    return stat_resize(v, n);
}

value llama_interpret(char* prog, size_t offset, size_t len) {
...
long llama_Long_val(value v) { return Long_val(v); }
double llama_Double_val(value v) { return Double_val(v); }
char* llama_String_val(value v) { return String_val(v); }

value llama_Val_long(long l) { return Val_long(l); }
value llama_copy_double(double d) { return copy_double(d); }
value llama_copy_string(char* s) { return copy_string(s); }

int llama_Is_block(value v) { return Is_block(v); }

unsigned int llama_obj_tag(value arg) {
    return Tag_val(arg);
}

size_t llama_Wosize_val(value v) { return Wosize_val(v); }
value llama_Field(value v, size_t i) { return Field(v, i); }
void llama_Store_field(value v, size_t i, value fv) { modify(&Field(v, i), fv); }

value llama_obj_new_block(unsigned int tg, size_t sz) {
    value res;
