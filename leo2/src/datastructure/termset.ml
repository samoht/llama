...
*)
      ...
      let _ =
        try (
          ...
          print_string ("while inserting node "^(string_of_int id)^": Appl_node("^(string_of_int id1)^","^(string_of_int id2)^"):\n"^s^"\n") in
      id
  | Bound_node(typ,i) ->
...
(*		let l =	match strct with
	...
	let rec draw_tree d t =
		...
		let l = List.sort (fun n n' -> compare (snd n') (snd n)) (Hashtbl.find idcount t.nodeid) in
...
let analyze_termset idx =
  ...
  let bindings =
    Hashtbl.fold
     (fun a b c -> let x = (a*(scaling-1))/max_bindings in
