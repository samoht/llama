...
let add_to_load_path dir =
  try
    let dir = expand_directory Depconfig.libdir dir in
...
let find_dependency modname (byt_deps, opt_deps) =
  try
    ...
    let optname =
      ...
      then basename ^ ".lmx"
      else basename ^ ".lmi" in
    ((basename ^ ".lmi") :: byt_deps, optname :: opt_deps)
  with Not_found ->
  try
    let filename = find_file (modname ^ ".ml") in
    let basename = Filename.chop_suffix filename ".ml" in
    let bytename =
      basename ^ (if !native_only then ".lmx" else ".lmo") in
    (bytename :: byt_deps, (basename ^ ".lmx") :: opt_deps)
  with Not_found ->
    (byt_deps, opt_deps)
  ...
  try
    ...
    if !raw_dependencies then begin
      ...
      let init_deps =
        ...
        then let lmi_name = basename ^ ".lmi" in ([lmi_name], [lmi_name])
        else ([], []) in
      let (byt_deps, opt_deps) =
        Set.fold find_dependency
                              !Depend.free_structure_names init_deps in
      print_dependencies (basename ^ ".lmo") byt_deps;
      print_dependencies (basename ^ ".lmx") opt_deps
    end;
...
let mli_file_dependencies source_file =
  ...
  try
    ...
    if !raw_dependencies then begin
      ...
      print_dependencies (basename ^ ".lmi") byt_deps
    end;
    close_in ic; remove_preprocessed input_file
  with x ->
    close_in ic; remove_preprocessed input_file; raise x
...
let file_dependencies_as kind source_file =
  Frontlocation.input_name := source_file;
  try
  ...
  with x ->
    ...
    | Lexer.Error(err, range) ->
        fprintf Format.err_formatter "@[%a%a@]@."
          Frontlocation.print_error range  Lexer.report_error err
    | Syntaxerr.Error err ->
...
let _ =
  ...
  Arg.parse [
     ...
     "-native", Arg.Set native_only,
       "  Generate dependencies for a pure native-code project (no .lmo files)";
