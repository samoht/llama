...
(*  print_string ((!mv)^"Insert-Arg-Function\n");*)
...
(*  print_string ((!mv)^"subst: "^(Term.to_string (Termset.retrieve idx.termbase t))^"\n");
  print_string ((!mv)^"  args: "^(string_of_int (List.length args)));
  print_string (", scope: "^(string_of_int scope));
  print_string (", shift: "^(string_of_int shift)^"\n");
  let mv_old=(!mv) in
  mv:="|"^(!mv);*)
(*  print_string ("subst': "^(Term.to_string (Termset.retrieve idx.termbase t))^"\n");
  List.iter (fun (a,b) -> print_string ((Term.to_string (Termset.retrieve idx.termbase a))^"/"^(Term.to_string (Termset.retrieve idx.termbase b))^",\n"))
            s;
  print_string "]\n";*)
...
(*  print_string "struct...";*)
  ...
  match (next_data occs) with
    WithData (f,d) -> (*print_string ((!mv)^"replace\n");*)
  ...
  | Appl_node (t1,t2) ->  (*print_string ((!mv)^"new_arg: "^(Term.to_string (Termset.retrieve idx.termbase t2))^"\n");*)
  ...
  in
(*  mv:=mv_old;
  print_string ((!mv)^"ret:   "^(Term.to_string (Termset.retrieve idx.termbase t_new))^"\n");
  print_string ((!mv)^"  from: "^(Term.to_string (Termset.retrieve idx.termbase t))^"\n");
  print_string ((!mv)^"  args: "^(string_of_int (List.length args)));
  print_string (", scope: "^(string_of_int scope));
  print_string (", shift: "^(string_of_int shift)^"\n");*)
  index_node t_new idx;
(*  print_string "indexed\n";
  print_string ("    --> "^(Term.to_string (Termset.retrieve idx.termbase t))^"\n[\n");*)
  t_new
