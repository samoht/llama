...
let is_typevar = function
    Basetype s as ty -> not (is_polyvar ty || s="o" || s="i") (* HACK! We should look at signature. *)
  | Funtype _ -> false
...
let rec occurs x = function
    Basetype s -> s=x
  | Funtype(ty1,ty2) -> occurs x ty1 or occurs x ty2
...
let uninst_polyvars ty vl =
  let replinv = combine vl (iteri id (length vl)) in
...
let rec to_hotptp = function
  ...
  | Funtype(ty1,ty2) -> (add_pars ty1)^">"^(add_pars ty2)
and add_pars ty =
  if is_basetype ty then to_hotptp ty
  else "("^(to_hotptp ty)^")"
*)
