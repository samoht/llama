...
let global_table =
  ref (new_numtable 1 : path numtable)
and literal_table =
...
let exn_tag_table = ref(new_numtable 1 : (path * int) numtable)
and tag_exn_table = ref( [| |] : (path * int) array )
and unknown_exn_name = ((Module "?", "?"), 0)
;;
